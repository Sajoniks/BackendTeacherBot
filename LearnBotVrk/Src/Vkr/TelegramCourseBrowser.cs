...
namespace LearnBotVrk.Vkr
{
    public class TelegramCourseBrowser : ICourseBrowser
    {
        ...
        public async Task<Message> SendReaderAsync(IBot bot, Chat chat)
        {
            ...
            return await bot.EditMessageTextAsync(chat, _browserMessage.Id, Reader.CurrentPageContent, builderRow.ToBuilder().Build());
        }
        ...
        public async Task<Message> SendParagraphListAsync(IBot bot, Chat chat)
        {
            if (Chapter != null)
            {
                ...
                _browserMessage =
                    await bot.EditMessageTextAsync(chat, _browserMessage.Id, $"Глава: \"{Chapter.Title}\"", builder.Build());
            }
